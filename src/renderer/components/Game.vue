...
var Board = function() {
    ...
    this.nums = []; // 用于来做逻辑数字
    // 初始化棋盘
    this.nums = new Array(this.SIZE);
    for (var row = 0; row < this.nums.length; row++) {
        this.nums[row] = new Array(this.SIZE);
        for (var col = 0; col < this.nums[row].length; col++) {
            // var num = Math.pow(2, parseInt(Math.random() * 10) + 1);
...
Board.prototype.rotateNums = function(n) {
    ...
    if (n === 0) return;
    var numsTemp = [];
    numsTemp = new Array(this.SIZE);
    for (var row = 0; row < numsTemp.length; row++) {
        numsTemp[row] = new Array(this.SIZE);
    }
    // 旋转90°
    // 1 2 3       7 4 1
    // 4 5 6  -->  8 5 2
    // 7 8 9       9 6 3
    var dst = this.SIZE - 1; // 这里我们从目标矩阵的最后一列开始存放数据
    for (var row = 0; row < this.SIZE; row++, dst--)
...
Board.prototype.canMove = function() {
    ...
    this.nums.forEach((rowNums, row) => {
        rowNums.forEach((item, col) => {
            ...
            dirs.forEach((dir) => {
                if (dir[0] >= 0 && dir[0] < this.SIZE && dir[1] >= 0 && dir[1] < this.SIZE) {
...
export default {
    ...
    methods: {
        ...
        keyDown(event) {
            if (event.keyCode >= 37 && event.keyCode <= 40 && this.arrowsKeyUp && (!this.board.win) && (!this.board.lost)) {
